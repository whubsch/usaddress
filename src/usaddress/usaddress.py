"""Functions for parsing and tagging addresses."""
...
def tag(
    ...
    """Tag an address string in its component parts.

    Args:
        address_string (str): The address string to tag
        tag_mapping (dict[str, str], optional): A dictionary of tag mappings to combine or rename fields.

    Returns:
        tuple[dict, str]: A dictionary of the tagged address and the address type
    """
    tagged_address: dict = {}
    ...
    return tagged_address, _get_address_type(og_labels, is_intersection)


def _get_address_type(labels: List[str], intersection: bool) -> str:
    """Get the address type

    Args:
        labels (List[str]): List of labels
        intersection (bool): Whether the address is an intersection

    Returns:
        str: The address type
    """
...
def tokenize(address_string: str) -> List[str]:
    ...
    ```python
    >>> tokenize("123 Main Street, Anytown, USA")
    ['123', 'Main', 'Street,', 'Anytown,', 'USA']
    ```
    Args:
        address_string (str): The address string to tokenize
...
def token_features(token: str) -> Dict[str, Any]:
    """Create features for a given token

    ```python
    >>> token_features("Street,")
    {'abbrev': False, 'digits': 'no_digits', 'word': 'street', 'trailing.zeros': False, 'length': 'w:6', 'endsinpunc': True, 'directional': False, 'street_name': True, 'has.vowels': True}    ```
    Args:
        token (str): The token to extract features from
...
def tokens_to_features(address: List[str]) -> List[Dict[str, Any]]:
...
def digits(
    ...
    """Check whether a token contains digits
    ...
    Returns:
        str: 'all_digits', 'some_digits', or 'no_digits'
    """
    if token.isdigit():
        return "all_digits"
    if token.isalpha():
...
def trailing_zeros(token: str) -> str:
    """Returns the trailing zeros of a token

    ```python
    >>> trailing_zeros("12309900")
    '00'
    >>> trailing_zeros("1775093")
    ''
    ```
